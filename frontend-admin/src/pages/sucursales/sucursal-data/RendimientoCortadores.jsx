    ...
    const loadCortadores = async () => {
      ...
      try {
        const res = await fetch(
          `${VITE_API_URL}/sucursales/${sucursal.id}/cortadores-carga`,
        );
        if (res.ok) {
          ...
          setCortadores(
            enhancedData.sort(
              (a, b) => b.pedidos_completados_hoy - a.pedidos_completados_hoy,
            ),
          );
        }
      } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cortadores.map((cortador, idx) => (
          <div
            key={cortador.id || idx}
            className="bg-white rounded-xl border border-neutral-200/80 shadow-2xs overflow-hidden transition-all hover:shadow-md"
          >
            <div
              className={`h-2 ${idx === 0 ? "bg-amber-400" : idx === 1 ? "bg-neutral-300" : idx === 2 ? "bg-amber-700" : "bg-blue-200"}`}
            ></div>
            <div className="p-5">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="size-12 rounded-full bg-neutral-100 flex items-center justify-center text-xl font-bold text-neutral-600">
                    {cortador.apodo
                      ? cortador.apodo.substring(0, 2).toUpperCase()
                      : cortador.nombre?.substring(0, 2).toUpperCase() || "CX"}
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 font-medium">
                      Carnicero Especialista
                    </p>
                  </div>
                </div>
                {idx < 3 && (
                  <Trophy
                    className={`size-6 ${idx === 0 ? "text-amber-400" : idx === 1 ? "text-neutral-400" : "text-amber-700"}`}
                  />
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-100">
                  ...
                  <p className="text-2xl font-black text-neutral-900">
                    {cortador.pedidos_completados_hoy}
                  </p>
                </div>
                ...
                <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-100">
                  ...
                  <p className="text-2xl font-black text-neutral-900">
                    {cortador.tiempo_promedio_corte}
                ...
                <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-100">
                  ...
                  <p className="text-2xl font-black text-main-blue">
                    {cortador.pedidos_en_corte}
                  </p>
                </div>
        ...
        {cortadores.length === 0 && (
          <div className="col-span-full p-8 text-center text-neutral-500 bg-white rounded-xl border border-neutral-200">
            No hay cortadores asignados en esta sucursal o no tienen actividad
            hoy.
          </div>
        )}
      </div>
