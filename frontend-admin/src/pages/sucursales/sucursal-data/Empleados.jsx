...
export default function Empleados() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingEmpleado
        ? `${VITE_API_URL}/empleados/${editingEmpleado.id}`
        : `${VITE_API_URL}/sucursales/${sucursal.id}/empleados`;
  ...
  const handleDelete = async (empId) => {
    if (
      !window.confirm("¿Seguro que deseás eliminar este integrante del equipo?")
    )
      return;
    try {
  ...
  const rolMeta = (rol) => {
    const map = {
      ...
      admin: {
        label: "Admin",
        color: "bg-neutral-800 text-white border-neutral-900",
        icon: Award,
      },
    };
    return (
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-500 mt-1">
                Gestioná los cortadores para las comandas, cajeras, encargados y
                cadetes de la sucursal.
              </p>
            </div>
          </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="animate-pulse bg-white border border-neutral-200 p-5 rounded-xl h-44"
            />
          ))}
        </div>
        ...
        <div className="text-center py-16 rounded-xl border bg-white border-neutral-200/80 shadow-2xs">
          ...
          <p className="text-xs opacity-60 mt-1">
            Hacé clic en "Agregar Integrante" para sumar personal a esta
            sucursal.
          </p>
        </div>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3">
                      <div className="size-11 rounded-xl bg-neutral-100 border border-neutral-200 flex items-center justify-center text-neutral-700 font-black text-sm shrink-0">
                        {emp.apodo
                          ? emp.apodo.slice(0, 2).toUpperCase()
                    ...
                    <span
                      className={`text-[11px] font-bold px-2.5 py-0.5 rounded-md border flex items-center gap-1 shrink-0 ${meta.color}`}
                    >
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-600 mt-0.5">
              Cimientos preparados para gestión de legajos digitales, adelantos
              de sueldo, ausencias y calendario de vacaciones.
            </p>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormValues({ ...formValues, apodo: e.target.value })
                  }
                  className="w-full px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400"
                />
                <p className="text-[11px] text-neutral-400 mt-1">
                  Este apodo aparecerá en la botonera *"¿Quién fracciona este
                  pedido?"*.
                </p>
              </div>
              ...
              {(formValues.rol === "encargado" ||
                formValues.rol === "admin" ||
                formValues.rol === "cajero") && (
              ...
              <div className="flex items-center gap-2 pt-2">
                <input
                  ...
                  onChange={(e) =>
                    setFormValues({ ...formValues, activo: e.target.checked })
                  }
                  className="size-4 rounded text-purple-600 focus:ring-purple-500"
                />
                <label
                  htmlFor="emp_activo"
                  className="font-bold text-neutral-800 cursor-pointer"
                >
